...
#include <string.h> // for memcpy (defined-behavior unaligned loads)
...
public:
  ...
  uint64_t hash() const
  {
    ...
    for (; data + 8 <= stop; data += 8)
      result = rotateLeft(result ^ processSingle(0, read64(data)), 27) * Prime1 + Prime4;

    // 4 bytes left ? => eat those
    if (data + 4 <= stop)
    {
      result = rotateLeft(result ^ read32(data) * Prime1,          23) * Prime2 + Prime3;
      data  += 4;
    }
...
private:
  ...
  /// read from unaligned memory: a plain pointer cast is undefined behavior
  /// for the misaligned inputs this class is fed (raw vertex data), while
  /// memcpy still compiles to one load instruction on x86/ARM
  ...
  static inline void process(const void* data, uint64_t& state0, uint64_t& state1, uint64_t& state2, uint64_t& state3)
  {
    ...
    state0 = processSingle(state0, read64(block));
    state1 = processSingle(state1, read64(block +  8));
    state2 = processSingle(state2, read64(block + 16));
    state3 = processSingle(state3, read64(block + 24));
  }
};
