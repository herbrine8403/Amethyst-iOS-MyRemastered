...
public:
  ...
  uint32_t hash() const
  {
    ...
    for (; data + 4 <= stop; data += 4)
      result = rotateLeft(result + read32(data) * Prime3,     17) * Prime4;
...
private:
  ...
  /// for misaligned inputs, while memcpy still compiles to one load
  ...
  static inline void process(const void* data, uint32_t& state0, uint32_t& state1, uint32_t& state2, uint32_t& state3)
  {
    ...
    state0 = rotateLeft(state0 + read32(block)      * Prime2, 13) * Prime1;
    state1 = rotateLeft(state1 + read32(block +  4) * Prime2, 13) * Prime1;
    state2 = rotateLeft(state2 + read32(block +  8) * Prime2, 13) * Prime1;
    state3 = rotateLeft(state3 + read32(block + 12) * Prime2, 13) * Prime1;
  }
};
